document.addEventListener('DOMContentLoaded', () => {
    ...
    const setupProject = async (project) => {
        ...
        try {
            // Passo 1: Tenta fazer uma leitura simples para ver se a tabela já existe.
            // Se falhar, não é um problema, apenas significa que ela provavelmente não existe.
            const { error: checkError } = await adminClient.from('keep_alive').select('id').limit(1);

            // Se a tabela não existir (checkError não é nulo)
            if (checkError) {
                log(`Tabela 'keep_alive' não encontrada. Tentando criar...`);
                connectionStatus.innerHTML = `Tabela não encontrada. Criando...`;

                // Passo 2: Chama a função 'execute_sql' que criamos no banco de dados.
                ...
                if (createError) {
                    // Se mesmo assim der erro, o problema é sério (ex: permissão, função não existe)
                    throw new Error(`Falha ao criar tabela: ${createError.message}. Verifique se a função 'execute_sql' foi criada no seu projeto Supabase.`);
                }
