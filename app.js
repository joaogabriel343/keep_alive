document.addEventListener('DOMContentLoaded', () => {
    ...
    const setupProject = async (project) => {
        ...
        try {
            ...
            if (checkError || tableData.length === 0) {
                ...
                log(`Tabela 'keep_alive' já existe. Verificando coluna 'mensagem'...`);
                const { data: columnData, error: columnError } = await adminClient.rpc('execute_sql_and_return', {
                    sql: `SELECT to_json(information_schema.columns) FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'keep_alive' AND column_name = 'mensagem';`
                });

                if (columnError || columnData.length === 0) {
                    log(`Coluna 'mensagem' não encontrada. Adicionando...`);
                    connectionStatus.innerHTML = `Atualizando tabela...`;
                    const addColumnSql = `ALTER TABLE public.keep_alive ADD COLUMN IF NOT EXISTS mensagem TEXT;`;
                    const { error: addColumnError } = await adminClient.rpc('execute_sql', { sql: addColumnSql });
                    if (addColumnError) throw new Error(`Falha ao adicionar coluna: ${addColumnError.message}.`);
                    log(`Coluna 'mensagem' adicionada com sucesso!`, 'success');
                    adminClient = getAdminClient(project, true);
                } else {
                    log(`Coluna 'mensagem' já existe.`);
                }
            }
