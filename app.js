document.addEventListener('DOMContentLoaded', () => {
    ...
    const setupProject = async (project) => {
        ...
        const { data, error } = await adminClient.from('information_schema.tables').select('table_name').eq('table_name', 'keep_alive').eq('table_schema', 'public');

        if (error) {
            log(`Erro ao verificar a tabela: ${error.message}`, 'error');
            connectionStatus.innerHTML = `<span style="color:red;">Falha na conexão: ${error.message}</span>`;
            return false;
        }

        if (data.length === 0) {
            log(`Tabela 'keep_alive' não encontrada. Tentando criar...`);
            connectionStatus.innerHTML = `Tabela não encontrada. Criando...`;
            const { error: createError } = await adminClient.rpc('query', {
                sql: `CREATE TABLE public.keep_alive (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, created_at TIMESTAMPTZ);`
            });

            if (createError) {
                log(`Falha ao criar a tabela: ${createError.message}`, 'error');
                connectionStatus.innerHTML = `<span style="color:red;">Falha ao criar tabela: ${createError.message}</span>`;
                return false;
            }
